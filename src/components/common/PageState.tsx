...
export function PageState(props: PageStateProps) {
  if (props.status === 'loading') {
    return (
      <div className="flex items-center gap-3 rounded-2xl border border-brand-100 bg-white p-4 text-sm text-slate-600">
  ...
  if (props.status === 'error') {
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-5">
        <p className="font-semibold text-red-900">Could not load this page</p>
  ...
  return (
    <div className="rounded-2xl border border-dashed border-brand-200 bg-white p-8 text-center">
      <p className="text-lg font-semibold text-brand-900">{props.title}</p>
      {props.description ? (
